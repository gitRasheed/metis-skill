...
5. Leave the campsite cleaner: delete dead code and scaffolding your change introduced, and pre-existing dead code your change makes obsolete — inside the code you already changed, not beyond it. Never commit generated artifacts (bytecode, build output) unless the repository explicitly tracks that artifact.
...
- Explicit tests: one behavior visible per test; branching and opaque control flow in a test body hide harness mistakes — parameterized cases are fine when each case stays independently identifiable.
...
2. Decide what correct behavior looks like before implementing; the red-green loop itself lives in Implementation rule 1.
...
8. Treat scaffolding tests as temporary. Before committing, review every new test critically: keep those that guard stable behavior, prevent a real regression, or fit the repo's test style; delete tests that only helped you think.
...
Sub-agent fan-out, only when all hold: the host instructions permit delegation, a sub-agent or task tool exists in your environment, and the diff is large (roughly more than 400 changed lines or 8 files):

- Spawn at most one agent per lens, max 4. Give each the diff plus the minimum surrounding code and contracts its lens needs to verify findings, its lens's checklist above, and the matching reference file from the list at the end of this skill — not the whole skill.
